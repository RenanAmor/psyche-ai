...
- **Recorrências** — `Recorrencia`, produzidas por `DetectorRecorrencias` e expostas em `/sujeitos/{id}/observacoes`. **Implementado** (Sprint 14/15) — ver detalhamento campo a campo (frequência, ocorrências, persistência) em [Representacao-Computacional/Recorrencias.md](../Representacao-Computacional/Recorrencias.md), que classifica alguns desses campos como "Parcialmente implementado" por não terem representação nomeada própria (ex.: "Intensidade").
...
- **Representações lacanianas** — os rótulos de `ReclassificadorLacaniano`: "deslize metonímico" (observação direta de Repetição) e "metáfora — condensação" (por ponte com o Motor Freud, quando o conteúdo é classificado como Chiste ou Sonho) — os dois rótulos lacanianos efetivamente produzidos hoje (corrigido na Sprint 30 — a afirmação anterior de que apenas Metonímia era produzida estava desatualizada em relação a `ReclassificadorLacaniano::reclassificarPorTipoFreudiano()`). **Implementado** (Sprint 16); demais rótulos lacanianos (cadeia significante, Outro, Falta, Objeto a, RSI, desejo lacaniano) sem representação computacional nesta versão (ver [Modelo-Observacional/README.md](../Modelo-Observacional/README.md)).
