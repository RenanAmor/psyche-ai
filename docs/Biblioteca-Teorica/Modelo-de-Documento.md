...
- **Motores do PsycheAI relacionados** — qual(is) dos motores conceituais do PsycheAI ([Documento-Mestre.md §7](../Documento-Mestre.md#7-arquitetura-conceitual): Discourse Engine, Freud Engine, Lacan Engine, ECO — Estrutura Computacional de Observação) usam ou poderão usar este conceito como fundamentação. "Modo Socrático" é o nome usado nos documentos catalogados antes da Sprint 28 para o mesmo componente hoje oficialmente chamado de ECO (ver nota de nomenclatura em [README.md](README.md#cadeia-de-rastreabilidade)). "Nenhum (catalogação apenas)" é um valor válido quando a obra ainda não tem uso computacional definido.
