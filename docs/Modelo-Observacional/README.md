...
- **Reclassificado (não observado por conta própria)**: [Metonímia](Conceitos/metonimia.md) — único rótulo lacaniano efetivamente produzido, sobre a mesma observação de Repetição.
- **Mapeado, nunca efetivamente produzido**: [Metáfora](Conceitos/metafora.md) — está na tabela de reclassificação, mas o detector atual não captura o fenômeno (substituição entre dois conteúdos distintos) que a fundamentaria.
