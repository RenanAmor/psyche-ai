...
final class GravacaoAudioMapper
{
    public static function save(PDO $pdo, GravacaoAudio $gravacao): void
    {
        $statement = $pdo->prepare(
            'INSERT INTO gravacoes_audio (id, sessao_id, caminho_armazenamento, status, criado_em, transcrita_em)
             VALUES (:id, :sessao_id, :caminho_armazenamento, :status, :criado_em, :transcrita_em)
    ...
    {
        $statement = $pdo->prepare(
            'SELECT id, sessao_id, caminho_armazenamento, status, criado_em, transcrita_em
